...
object CurlApi {
  ...
  def globalInit(flags: Long): CurlCode = CCurl.globalInit(flags)
  ...
  implicit class CurlHandleOps(handle: CurlHandle) {
    ...
    def perform: CurlCode = {
      println("before")
      val x = CurlCode(CCurl.perform(handle))
      println("after " + x)
      x
    }
    ...
    def option(option: CurlOption, parameter: String)(implicit z: Zone): CurlCode =
      setopt(handle, option, toCString(parameter, StandardCharsets.UTF_16))
